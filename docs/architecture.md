...
The AI feature uses an adapter abstraction so the rest of CodeAudit does not depend directly on a provider SDK.
...
- `GEMINI_TIMEOUT_MS` — optional positive timeout in milliseconds, defaulting to 15000.

The provider requests structured JSON containing:

- `indicator` — normalized value from 0 to 1;
- `confidence` — normalized value from 0 to 1;
- `observations` — concrete categorized observations.

The internal result model additionally exposes `available`, `provider`, `label`, `disclaimer`, and an optional `error`. Indicator labels use the current deterministic boundaries:

- `< 0.34` → `low`;
- `< 0.67` → `medium`;
- otherwise → `high`.

When the provider is not configured, returns invalid data, times out, or raises an API error, the provider returns a controlled `unavailable` result rather than crashing the application.

The AI result is explicitly probabilistic. It must not be represented as proof of authorship or as a scientifically validated probability that a specific percentage of code was generated by AI.
